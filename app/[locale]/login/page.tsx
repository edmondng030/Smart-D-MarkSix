...
export default async function LoginPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ error?: string; message?: string; next?: string }> }) {
  const [{ locale }, query] = await Promise.all([params, searchParams]);
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const next=query.next?.startsWith(`/${locale}/`)&&!query.next.startsWith(`//`)?query.next:`/${locale}/admin/imports`;
  ...
  return <PageShell icon={LockKeyhole} eyebrow="Secure access" title="管理員登入" description="登入後才可匯入、審核及發布資料。">
    <div className="mt-10 max-w-lg rounded-2xl border border-white/10 bg-[#141b2d]/80 p-6 sm:p-8">
      ...
      <p className="mt-5 text-xs leading-6 text-slate-500">只限已獲授權使用者。登入操作由 Supabase Auth 處理，網站不會記錄你的密碼。</p>
    </div>
  </PageShell>;
}
